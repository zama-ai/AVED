 ...
 * @ctxt: void ptr containing the AMC control context.
 *
               ...
               uint32_t            data_size)
{
    ...
    case AMC_CMD_ID_PEEKPOKE:
    {
        struct amc_proxy_ami_peek_poke peek_poke_req = { 0 };

        peek_poke_req.address = payload_address;
        peek_poke_req.length  = payload_size;
        peek_poke_req.type    = PEEKPOKE_GET_TYPE(flags);
    ...
    case AMC_CMD_ID_PEEKPOKE:
    {
        ret = amc_proxy_get_response_peek_poke(amc_proxy_cmd);
        if (!ret) {
            if (PEEKPOKE_GET_TYPE(flags) == AMC_PROXY_CMD_RW_REQUEST_READ) {
                memcpy_gcq_payload_from_device(amc_ctrl_ctxt, payload_address, data_buf, data_size);
            }
        }
        break;
          ...
          void            *event_cb_data)
{
    ...
    *amc_ctrl_ctxt = kzalloc(sizeof(struct amc_control_ctxt), GFP_KERNEL);
    ...
    AMI_VDBG((*amc_ctrl_ctxt), "Successfully created FW_IF_GCQ");

    /* Init proxy and bind in callback */
    ret = amc_proxy_init(0, (*amc_ctrl_ctxt));
    if (ret) {
        DEV_ERR(dev, "amc_proxy_init() failed %d", ret);
        goto fail;
    }
    AMI_VDBG((*amc_ctrl_ctxt), "Successfully initialized AMC proxy");
    ...
    AMI_VDBG((*amc_ctrl_ctxt), "Successfully bound amc_proxy_callback");
