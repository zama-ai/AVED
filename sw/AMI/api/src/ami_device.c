...
// Number of physical functions, hardcoded to 2
#define PF_NB                   (2)
/*****************************************************************************/
 ...
 * Return: AMI_STATUS_OK or AMI_STATUS_ERROR.
 */
static int pci_remove(uint16_t bdf, uint8_t pf_nb);
...
int ami_dev_pci_reload(ami_device **dev, const char *bdf)
{
    ...
    if (pci_remove(bdf_num, PF_NB) == AMI_STATUS_OK) {
